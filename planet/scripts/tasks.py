...
def hopper_run(config, params):
  ...
  state_components = [
      ...
      'torso_vertical', 'velocity']
  env_ctor = functools.partial(
      _marathon_env, action_repeat, max_length, 'MarathonHopperEnv-v0', 'run', worker_id)
  return Task('hopper_run', env_ctor, max_length, state_components)
...
def _marathon_env(action_repeat, max_length, domain, task, worker_id):
